...
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778712399` covers Dusk source ref `eff5e3bc181707756eead42880c71ccd1e685d34` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; the monorepo ref is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. Subsequent Dusk docs/report/guard commits are outside `DUSK_REPRO_COVERED_PATHS`, and the production-readiness guard reports `coveredPathDelta: none`. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF` and report `monorepoCoveredPathDelta`. Moving PR head and check URLs are intentionally read from the live PR check rollups rather than pinned in this audit row. |
...
| Recent head and gate refresh | Active Dusk PR #1, monorepo PR #1, and sign-off issue #2 bodies each include a `head and gate refresh:` handoff entry; recorded 2026-05-14 `make gate-status` samples reported no untracked source, upstream base `7689ff65f4929a72ad0650a03e8dd7d987f0e802`, `upstreamOpenPrsFromDuskHead: 0`, `coveredPathDelta: none`, `monorepoCoveredPathDelta: none`, and the same review/sign-off/runner/token blockers. The latest post-push lightweight review-gate handoff is https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4445875247 for Dusk head `15f7e0dccc8472648d4f9583c86bcbc1286fe07d`, review export `/tmp/hyperlane-review-export-1778713607`, and dispatcher merge-order log `/tmp/hyperlane-merge-order-logs.GvcDla`. The sample at Dusk head `6f6854d3f8ec797cf21afda2c4df482fb42bea16` had `Dusk review policy gate` passing at https://github.com/dusk-network/hyperlane-dusk/actions/runs/25830381479/job/75893688438 and `Production readiness guard` failing closed at https://github.com/dusk-network/hyperlane-dusk/actions/runs/25830381459/job/75893688355. A later docs-only sample at Dusk head `c1388203d2757d8a47a23ccb9db3154c51a187e3` had the same expected shape: `Dusk review policy gate` passed at https://github.com/dusk-network/hyperlane-dusk/actions/runs/25830505657/job/75894085847 and `Production readiness guard` failed closed at https://github.com/dusk-network/hyperlane-dusk/actions/runs/25830505654/job/75894085922. Live values are read with `make gate-status` and `gh pr view 1 --repo dusk-network/hyperlane-dusk --json headRefOid,statusCheckRollup,reviewDecision,mergeable,state`. |
