...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `7689ff65`; latest clean-layout `make repro-check-agent` result `1778712399` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4445830856 tied to their test coverage; `make report-hygiene` rejects stale decision-record links to the superseded clean-layout evidence comment |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; artifact scans reject secret-like key/pem filenames plus `.env`/`.env.*` files before upload and reject inline `key`, `privateKey`, or `private_key` 32-byte hex fields in JSON, TOML, and YAML-style assignments plus uppercase env/log key assignments; `make fail-closed-self-test` covers `.env.bridge`, JSON `privateKey`, TOML `private_key`, single-quoted YAML `privateKey`, and `DUSK_SIGNER_KEY` artifact rejection; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4445830856 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
...
| Latest clean-layout repro evidence | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-warp-unwrap-repro-1778712399.tgz`; SHA256 `56b3659a46d76b9becac859fb6737af57f6a5b3eef76f13c631e792097490a17`; contains `/tmp/hyperlane-warp-unwrap-repro-1778712399.log` with SHA256 `814018acb922b7d27b9419837916986381bd9a729b56d3a9f6369466b008b8c0`; contains clean-layout repro run `1778712399` for Dusk source ref `eff5e3bc181707756eead42880c71ccd1e685d34` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; `scripts/secret-hygiene-check.sh` passed inside the repro and over the archive staging directory and tarball, and `scripts/archive-hygiene-check.sh` passed over the durable evidence archive directory after the archive was added. GitHub-facing evidence comment: https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4445830856. Previous checkout-v6 evidence remains in issue #2 history. |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778712399` covers Dusk source ref `eff5e3bc181707756eead42880c71ccd1e685d34` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; the monorepo ref is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. `DUSK_REPRO_COVERED_PATHS` includes runtime, tooling, demo, and VM integration test source (`contracts types data-driver dusk-tx e2e wasm-bindings demo tests Cargo.toml Cargo.lock`) so test-source changes cannot be hidden from repro-delta reporting. Subsequent Dusk docs/report/guard commits are outside that covered path set, but the malformed warp-token VM test slice in `tests/tests/integration.rs` is a covered-path change and must remain visible as `coveredPathDelta: present` until a new clean-layout repro covers the updated 72-test suite. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF` and report `monorepoCoveredPathDelta`. Moving PR head and check URLs are intentionally read from the live PR check rollups rather than pinned in this audit row. |
...
| Recent head and gate refresh | Active Dusk PR #1, monorepo PR #1, sign-off issue #2, and split decision issue bodies point at the current gate handoff https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446455754 and the reviewer action queue https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446422402. A 2026-05-14 live refresh after the malformed warp-token VM test slice observed Dusk PR head `f0c5d2ef13a6cbfcaaadb129566053d008969159`; `Dusk review policy gate` passed at https://github.com/dusk-network/hyperlane-dusk/actions/runs/25836065840/job/75911308870 and `Production readiness guard` failed closed at https://github.com/dusk-network/hyperlane-dusk/actions/runs/25836065848/job/75911308832. The latest `make review-hygiene` export was `/tmp/hyperlane-review-export-1778721837`; `make gate-status` reported no untracked source, upstream base `7689ff65f4929a72ad0650a03e8dd7d987f0e802`, `upstreamOpenPrsFromDuskHead: 0`, `missingRequiredStatusChecks: none`, `coveredPathDelta: present` for `tests/tests/integration.rs`, `monorepoCoveredPathDelta: none`, no visible `DUSK_ORG_READ_TOKEN` in either internal repo, no visible repo-level `dusk-hyperlane` runner, and the same review/sign-off/runner/token blockers. Moving PR heads and check URLs are intentionally read live with `make gate-status`, `make gate-status-fresh`, and `gh pr view 1 --repo dusk-network/hyperlane-dusk --json headRefOid,statusCheckRollup,reviewDecision,mergeable,state` instead of being treated as static completion proof. |
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4445830856 |
