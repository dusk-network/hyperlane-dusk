...
`e6afc9d0a4adfe457d1775b6b073e9baa4974137` rebases onto upstream
...
`e6afc9d0a4adfe457d1775b6b073e9baa4974137` also updates the Dusk agent gate
to `actions/checkout@v6` after the tag was verified through the GitHub API.
...
- Dusk PR is ready for review and open; review is requested from `moCello`.
  Recent CI evidence at head `60ff8bad100a9a7c1ffb3548b21a7e78bba2bcfc` has
  `Dusk review policy gate` passing at
  https://github.com/dusk-network/hyperlane-dusk/actions/runs/25814505897/job/75839305375
...
- Manual workflow dispatcher PR #3 is ready for review, open, has
  ...
  head `d7a0a35ebc01a8790ca252fac215608b76559190` merge cleanly in either
...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `7689ff65`; latest clean-layout `make repro-check-agent` result `1778694794` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4443848247 tied to their test coverage |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4443848247 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
...
| Latest checkout-v6 clean-layout repro evidence | `/tmp/hyperlane-checkout-v6-repro-1778694794.log`; SHA256 `1f3c523278c8a1fcbb31600d88851fc366113559d1eb3684a7190876ab8c05ca`; contains clean-layout repro run `1778694794` for Dusk source ref `3d9008556ff2d28f3545280f6508eb1ea802e1ba` and monorepo ref `e6afc9d0a4adfe457d1775b6b073e9baa4974137`; evidence comment https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4443848247 |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778694794` covers Dusk source ref `3d9008556ff2d28f3545280f6508eb1ea802e1ba` and monorepo ref `e6afc9d0a4adfe457d1775b6b073e9baa4974137`; the current monorepo PR head is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. Subsequent Dusk docs/report commits are outside `DUSK_REPRO_COVERED_PATHS`, and the production-readiness guard reports `coveredPathDelta: none`. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF`; current output reports `monorepoCoveredPathDelta: none`. |
...
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4443848247 |
