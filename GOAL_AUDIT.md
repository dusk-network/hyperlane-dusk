...
`3b7d9f64d7d9465eaa868770d970243d98bc53c6` rebases onto upstream
...
- The 2026-05-14 `make review-gates` refresh passed at Dusk head
  `2dd0d227cf0c33033cd9206151c1cbca6cddfffb`, monorepo head
  `3b7d9f64d7d9465eaa868770d970243d98bc53c6`, and dispatcher head
...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `a8c9430`; latest clean-layout `make repro-check-agent` result `1778751867` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449591043 tied to their test coverage; `make report-hygiene` rejects stale decision-record links to the superseded clean-layout evidence comment |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; artifact scans reject secret-like key/pem filenames plus `.env`/`.env.*` files before upload and reject inline `key`, `privateKey`, or `private_key` 32-byte hex fields in JSON, TOML, and YAML-style assignments plus uppercase env/log key assignments; `make fail-closed-self-test` covers `.env.bridge`, JSON `privateKey`, TOML `private_key`, single-quoted YAML `privateKey`, and `DUSK_SIGNER_KEY` artifact rejection; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449591043 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
...
| Recent head and gate refresh | Active Dusk PR #1, monorepo PR #1, sign-off issue #2, and split decision issue bodies point at latest clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449591043 and the current gate handoff https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449346805. A 2026-05-14 current-head repro run `1778751867` tested Dusk source ref `2dd0d227cf0c33033cd9206151c1cbca6cddfffb`, monorepo ref `3b7d9f64d7d9465eaa868770d970243d98bc53c6`, upstream base `a8c9430c82f0b66faf4231828f798dcbec95dfab`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`. Latest aggregate `make review-gates` passed with review-hygiene export `/tmp/hyperlane-review-export-1778752305` and dispatcher merge-order log `/tmp/hyperlane-merge-order-logs.q2c56I`; the current issue #2 gate handoff, reviewer queue, and #8 runbook reference that aggregate evidence, and `make review-hygiene` passed after those GitHub-only edits with export `/tmp/hyperlane-review-export-1778752305`. A fresh upstream check confirmed Hyperlane upstream base `a8c9430c82f0b66faf4231828f798dcbec95dfab` is the merge-base and the branch remains `45 0` ahead/behind. `make production-readiness-guard` failed closed on the expected blockers: open/review-required Dusk, monorepo, and dispatcher PRs; 7 unchecked issue #2 sign-off items; split decision issues #4-#9 open; no visible repo-level `dusk-hyperlane` runner; and no visible `DUSK_ORG_READ_TOKEN` in either internal repo. Moving PR heads and check URLs are intentionally read live with `make gate-status`, `make gate-status-fresh`, and `gh pr view 1 --repo dusk-network/hyperlane-dusk --json headRefOid,statusCheckRollup,reviewDecision,mergeable,state` instead of being treated as static completion proof. |
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449591043 |
