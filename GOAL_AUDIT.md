...
`b924b62a62bd9636ea4b60507fe281c36e7a5a43` rebases onto upstream
`a8c9430c82f0b66faf4231828f798dcbec95dfab`. It retains the Dusk agent gate
and Dusk review-policy gate `actions/checkout@v6` updates after the tag was
...
- Upstream Hyperlane `main` and the monorepo branch merge-base both resolve to
  `a8c9430c82f0b66faf4231828f798dcbec95dfab`.
...
- The 2026-05-14 `make review-gates` refresh passed at Dusk head
  `7042d4460e0b01ed08de8cafa3fde46d61a328a8`, monorepo head
  `b924b62a62bd9636ea4b60507fe281c36e7a5a43`, and dispatcher head
  ...
  `/tmp/hyperlane-merge-order-logs.WdLtsi`, verified no covered-path delta from
...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `a8c9430`; latest clean-layout `make repro-check-agent` result `1778748796` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449213312 tied to their test coverage; `make report-hygiene` rejects stale decision-record links to the superseded clean-layout evidence comment |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; artifact scans reject secret-like key/pem filenames plus `.env`/`.env.*` files before upload and reject inline `key`, `privateKey`, or `private_key` 32-byte hex fields in JSON, TOML, and YAML-style assignments plus uppercase env/log key assignments; `make fail-closed-self-test` covers `.env.bridge`, JSON `privateKey`, TOML `private_key`, single-quoted YAML `privateKey`, and `DUSK_SIGNER_KEY` artifact rejection; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449213312 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
...
| Latest clean-layout repro evidence | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-clean-repro-current-head-1778748796.tgz`; SHA256 `23035c56a53fe83333deb8c3e8df0fccd8c1abc16fca61ae5b99187118b4f232`; contains `/tmp/hyperlane-clean-repro-current-head-1778748796.log` with SHA256 `48a502feed7cf3a49948fd8e83439d7490327e8cbe623dcb7ff3acbd3942d652`; contains clean-layout repro run `1778748796` for Dusk source ref `7042d4460e0b01ed08de8cafa3fde46d61a328a8`, monorepo ref `b924b62a62bd9636ea4b60507fe281c36e7a5a43`, and upstream base `a8c9430c82f0b66faf4231828f798dcbec95dfab`; `scripts/secret-hygiene-check.sh` passed inside the repro, and `scripts/archive-hygiene-check.sh` passed over the durable evidence archive directory after the archive was added. GitHub-facing evidence comment: https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449213312. Previous checkout-v6 evidence remains in issue #2 history. |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778748796` covers Dusk source ref `7042d4460e0b01ed08de8cafa3fde46d61a328a8` and monorepo ref `b924b62a62bd9636ea4b60507fe281c36e7a5a43`; the monorepo ref is rebased onto upstream `a8c9430c82f0b66faf4231828f798dcbec95dfab`. `DUSK_REPRO_COVERED_PATHS` includes runtime, tooling, demo, and VM integration test source (`contracts types data-driver dusk-tx e2e wasm-bindings demo tests Cargo.toml Cargo.lock`) so test-source changes cannot be hidden from repro-delta reporting. The malformed warp-token VM test slice in `tests/tests/integration.rs` remains covered by the prior run `1778722626`; the current-head repro covers the same Dusk runtime/test path set plus the rebased monorepo agent head. `make gate-status-fresh` and `make production-readiness-guard` compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF` and report `monorepoCoveredPathDelta`. Moving PR check URLs are intentionally read from the live PR check rollups rather than pinned in this audit row. |
