...
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778695627` covers Dusk source ref `ef8ee43cd99569299b9744b498ac1bbac69950bc` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; the current monorepo PR head is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. Subsequent Dusk docs/report commits are outside `DUSK_REPRO_COVERED_PATHS`, and the production-readiness guard reports `coveredPathDelta: none`. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF`; current output reports `monorepoCoveredPathDelta: none`. |
