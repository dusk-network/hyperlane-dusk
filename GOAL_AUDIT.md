...
|---|---|---|---|
| `dusk-network/hyperlane-dusk` | `feat/dusk-hardening-v2` | Live PR head is checked through GitHub and `make gate-status`; latest clean-layout `make repro-check-agent` evidence is run `1778577847`, which tested Dusk `06dbf75e2d67b0bbc5aa450066bfb5743f79bdd2`, monorepo `a44020dc998b7fe868254a5d1a349b9eb8ded899`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`. Later Dusk commits add audit/test-report synchronization and repeatable GitHub review-hygiene guardrails on top. Earlier commits cover implementation, cleanup regression guards, audit, CI/repro, signer, default-branch, cross-repo handoff docs, generated signer key cleanup, gate-status reporting, split decision routing, reviewer evidence links, Mailbox fee-overflow hardening, Mailbox nonce-overflow hardening, fee-accounting overflow hardening, targeted wasm clippy evidence for the production contract/type surface, and the repeatable `make clippy-contracts` wrapper in local repro | Internal PR ready for review, open, mergeable; review requested from `moCello`; labels `need:feedback`, `type:feature`; no status checks configured |
