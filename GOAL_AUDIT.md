...
`9e8c7abf054fcf1193abc81a2d984fe59b08eb16`. That slice removed Dusk agent
...
hyperlane-base`, `actionlint .github/workflows/dusk-agent-gate.yml`, and the
local Dusk agent panic/placeholder scan passed. CI `Dusk review policy gate`
passed on that head; CI `Dusk agent cargo check` passed the hardened scan and
still fails at the expected private companion-repo preflight until
`DUSK_ORG_READ_TOKEN` is provisioned.
...
| Avoid runtime `todo!`, `unimplemented!`, and direct `panic!` paths | `TEST_REPORT.md` records the Dusk contract/runtime/tooling scan command and result for `rg -n "todo!|unimplemented!|panic!" contracts types data-driver dusk-tx e2e wasm-bindings demo -g '!target'` with no matches in scoped production paths. `make gate-status` now also scans the tracked Dusk repo paths `contracts types data-driver dusk-tx e2e wasm-bindings demo` plus the monorepo `rust/main/chains/hyperlane-dusk` crate for direct placeholder/panic macros. The monorepo head `9e8c7abf054fcf1193abc81a2d984fe59b08eb16` additionally removes the remaining Dusk agent `expect(...)` paths from `rust/main/chains/hyperlane-dusk/src`; local `git grep -n -E 'todo!|unimplemented!|panic!|expect\(' -- rust/main/chains/hyperlane-dusk/src` and the updated CI scan report no matches before the CI job reaches the expected private Dusk checkout token preflight. The monorepo `docs/dusk-upstream-compatibility-review.md` records no added placeholder macros in the Dusk diff against `upstream/main`. | Done for internal review |
