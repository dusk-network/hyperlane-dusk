...
`99f718dfca395076285d3cc1226128599d67148c` rebases onto upstream
...
- The 2026-05-14 `make review-gates` refresh passed at Dusk head
  `ae215d061d22924dfeea64e90b4cb4803604aac4`, monorepo head
  `99f718dfca395076285d3cc1226128599d67148c`, and dispatcher head
...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `a8c9430`; latest clean-layout `make repro-check-agent` result `1778750702` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449464522 tied to their test coverage; `make report-hygiene` rejects stale decision-record links to the superseded clean-layout evidence comment |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; artifact scans reject secret-like key/pem filenames plus `.env`/`.env.*` files before upload and reject inline `key`, `privateKey`, or `private_key` 32-byte hex fields in JSON, TOML, and YAML-style assignments plus uppercase env/log key assignments; `make fail-closed-self-test` covers `.env.bridge`, JSON `privateKey`, TOML `private_key`, single-quoted YAML `privateKey`, and `DUSK_SIGNER_KEY` artifact rejection; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449464522 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
...
| Latest clean-layout repro evidence | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-clean-repro-current-head-1778750702.tgz`; SHA256 `090ed23b59f4a29d5f498fea9019c55164180876ce37a94729128d3ad389a5eb`; contains `/tmp/hyperlane-clean-repro-current-head-1778750702.log` with SHA256 `4169a677e58c6366c66d55e6ded684c4d8f095f99795de5f57e2f94868c0f8fc`; contains clean-layout repro run `1778750702` for Dusk source ref `ae215d061d22924dfeea64e90b4cb4803604aac4`, monorepo ref `99f718dfca395076285d3cc1226128599d67148c`, and upstream base `a8c9430c82f0b66faf4231828f798dcbec95dfab`; `scripts/secret-hygiene-check.sh` passed inside the repro, and `scripts/archive-hygiene-check.sh` passed over the durable evidence archive directory after the archive was added. GitHub-facing evidence comment: https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449464522. Previous checkout-v6 evidence remains in issue #2 history. |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778750702` covers Dusk source ref `ae215d061d22924dfeea64e90b4cb4803604aac4` and monorepo ref `99f718dfca395076285d3cc1226128599d67148c`; the monorepo ref is rebased onto upstream `a8c9430c82f0b66faf4231828f798dcbec95dfab`. `DUSK_REPRO_COVERED_PATHS` includes runtime, tooling, demo, and VM integration test source (`contracts types data-driver dusk-tx e2e wasm-bindings demo tests Cargo.toml Cargo.lock`) so test-source changes cannot be hidden from repro-delta reporting. The malformed warp-token VM test slice in `tests/tests/integration.rs` remains covered by the prior run `1778722626`; the current-head repro covers the same Dusk runtime/test path set plus the rebased monorepo agent head. `make gate-status-fresh` and `make production-readiness-guard` compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF` and report `monorepoCoveredPathDelta`. Moving PR check URLs are intentionally read from the live PR check rollups rather than pinned in this audit row. |
...
| Recent head and gate refresh | Active Dusk PR #1, monorepo PR #1, sign-off issue #2, and split decision issue bodies point at latest clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449464522 and the current gate handoff https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449346805. A 2026-05-14 current-head repro run `1778750702` tested Dusk source ref `ae215d061d22924dfeea64e90b4cb4803604aac4`, monorepo ref `99f718dfca395076285d3cc1226128599d67148c`, upstream base `a8c9430c82f0b66faf4231828f798dcbec95dfab`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`. Latest aggregate `make review-gates` passed with review-hygiene export `/tmp/hyperlane-review-export-1778751323` and dispatcher merge-order log `/tmp/hyperlane-merge-order-logs.e7I0Vo`; the current issue #2 gate handoff, reviewer queue, and #8 runbook reference that aggregate evidence, and `make review-hygiene` passed after those GitHub-only edits with export `/tmp/hyperlane-review-export-1778751323`. A fresh upstream check confirmed Hyperlane upstream base `a8c9430c82f0b66faf4231828f798dcbec95dfab` is the merge-base and the branch remains `45 0` ahead/behind. `make production-readiness-guard` failed closed on the expected blockers: open/review-required Dusk, monorepo, and dispatcher PRs; 7 unchecked issue #2 sign-off items; split decision issues #4-#9 open; no visible repo-level `dusk-hyperlane` runner; and no visible `DUSK_ORG_READ_TOKEN` in either internal repo. Moving PR heads and check URLs are intentionally read live with `make gate-status`, `make gate-status-fresh`, and `gh pr view 1 --repo dusk-network/hyperlane-dusk --json headRefOid,statusCheckRollup,reviewDecision,mergeable,state` instead of being treated as static completion proof. |
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4449464522 |
