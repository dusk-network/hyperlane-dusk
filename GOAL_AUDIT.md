...
| Dusk VM contract/type checks | `make all`; `make clippy-contracts` for the production contract/type surface -> passed; `cargo test -p hyperlane-dusk-types` -> 28 passed; `cargo test -p hyperlane-dusk-integration-tests` -> 70 passed; `cargo test -p dusk-tx` -> 3 passed; latest clean-layout `make repro-check-agent` result `1778596710` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `66e8c1f4`; latest clean-layout `make repro-check-agent` result `1778596710` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Latest clean-layout repro evidence archive | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-final-head-repro-20260512T144314Z.tgz`; SHA256 `e8c17cb43da130091a52b2c711aa64d0fef000b599997a372c968970c52dfa7e`; contains clean-layout repro run `1778596710`; `scripts/secret-hygiene-check.sh` passed over the archive staging directory and tarball |
