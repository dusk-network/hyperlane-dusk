...
| Recent head and gate refresh | Active Dusk PR #1, monorepo PR #1, sign-off issue #2, and split decision issue bodies point at latest clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484 and the current gate handoff https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446455754. A 2026-05-14 live refresh after promoting repro run `1778722626` observed Dusk PR head `222ae14469a960403699f5507f72be7c52321b29`; `make review-hygiene` passed with export `/tmp/hyperlane-review-export-1778723293`; `make gate-status` reported no untracked source, upstream base `7689ff65f4929a72ad0650a03e8dd7d987f0e802`, `upstreamOpenPrsFromDuskHead: 0`, `missingRequiredStatusChecks: none`, `coveredPathDelta: none`, `monorepoCoveredPathDelta: none`, no visible `DUSK_ORG_READ_TOKEN` in either internal repo, no visible repo-level `dusk-hyperlane` runner, and the same review/sign-off/runner/token blockers. `make production-readiness-guard` failed closed on those expected blockers. Moving PR heads and check URLs are intentionally read live with `make gate-status`, `make gate-status-fresh`, and `gh pr view 1 --repo dusk-network/hyperlane-dusk --json headRefOid,statusCheckRollup,reviewDecision,mergeable,state` instead of being treated as static completion proof. |
