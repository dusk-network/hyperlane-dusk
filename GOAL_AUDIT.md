...
| Dusk VM contract/type checks | `make all`; `make clippy-contracts` for the production contract/type surface -> passed; `cargo test -p hyperlane-dusk-types` -> 28 passed; `cargo test -p hyperlane-dusk-integration-tests` -> 70 passed; `cargo test -p dusk-tx` -> 3 passed; latest clean-layout `make repro-check-agent` result `1778607202` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `c6bce706`; latest clean-layout `make repro-check-agent` result `1778607202` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
| Local EVM <-> Dusk with TestMock/null-style ISM | Clean Rusk run `1778520709`: bidirectional E2E passed; review-head clean-Rusk run `1778587094` passed; current live-head clean-Rusk run `1778609411` passed at Dusk `b1ccdc9d1e7797bba4939405200aa4cc5aff2ea8` and monorepo `1f9e49fd9f0ba84ea93e472ddbd31fddd9a04cc3` | Done |
| Local EVM <-> Dusk with MessageIdMultisigISM | Clean Rusk run `1778521018`: bidirectional E2E with validator/checkpoint passed; review-head clean-Rusk run `1778587351` passed; current live-head clean-Rusk run `1778609697` passed at Dusk `b1ccdc9d1e7797bba4939405200aa4cc5aff2ea8` and monorepo `1f9e49fd9f0ba84ea93e472ddbd31fddd9a04cc3` | Done |
| Bidirectional DUSK/wDUSK bridge through relayer/validator, no manual process calls | `TEST_REPORT.md` records relayer/validator-driven EVM -> Dusk and Dusk -> EVM flows for TestMock and MessageIdMultisig; current live-head runs `1778609411` and `1778609697` passed both directions against clean Rusk after the final upstream rebase | Done |
| E2E freshness against current runtime source | `TEST_REPORT.md` records current live-head clean-Rusk TestMock and MessageIdMultisig E2E runs `1778609411` and `1778609697` at Dusk `b1ccdc9d1e7797bba4939405200aa4cc5aff2ea8`, monorepo `1f9e49fd9f0ba84ea93e472ddbd31fddd9a04cc3`, upstream base `c6bce706316206ac7b5652155c9ea92e96f78c39`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`; later documentation-only commits may move the Dusk PR head without changing tested runtime source | Done |
...
| Latest clean-layout repro evidence archive | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-current-head-repro-1778607202.tgz`; SHA256 `cc34e8e31abcab6ef3ef05587f38ce7963257586c30375d59b46c25296e0b2a1`; contains clean-layout repro run `1778607202` for Dusk source ref `836ee7d8d8e95152b3daaeebbc3fb56b0cc8e253` and monorepo ref `1f9e49fd9f0ba84ea93e472ddbd31fddd9a04cc3`; `scripts/secret-hygiene-check.sh` passed over the repro log and tarball |
| Live-head delta after latest clean-layout repro | The live Dusk PR head is checked through GitHub and `make gate-status`. From tested Dusk source ref `836ee7d8d8e95152b3daaeebbc3fb56b0cc8e253` to the live PR head, `git diff --name-status` reports only `GOAL_AUDIT.md`, `TEST_REPORT.md`, and `scripts/github-review-hygiene.sh` as of the latest gate refresh. The commits are evidence docs plus review-hygiene guard updates, including `26c2850`, `45f62b9`, `48907f3`, `b1ccdc9`, `894f9a3`, `0c0e665`, and `a886af0`. Current live-head E2E runs `1778609411` and `1778609697` tested Dusk `b1ccdc9d1e7797bba4939405200aa4cc5aff2ea8` directly before the later docs/tooling-only commits. |
...
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4433179148 |
