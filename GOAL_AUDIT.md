...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `7689ff65`; latest clean-layout `make repro-check-agent` result `1778695627` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Latest checkout-v6 clean-layout repro evidence | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-checkout-v6-repro-1778695627.tgz`; SHA256 `1f16dd8caa86c54ff351f0a0fc41f9ee8083c25515514ac77afb2f60f7483ccb`; contains `/tmp/hyperlane-review-checkout-v6-repro-1778695627.log` with SHA256 `9384e858bdd00f88665969a91cfa583048c4437177b615c7ddf49c676a4c2c12`; contains clean-layout repro run `1778695627` for Dusk source ref `ef8ee43cd99569299b9744b498ac1bbac69950bc` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; `scripts/secret-hygiene-check.sh` passed over the archive staging directory and tarball, and `scripts/archive-hygiene-check.sh` passed over the durable evidence archive directory after the archive was added; evidence comment https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4443963744 |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778695627` covers Dusk source ref `ef8ee43cd99569299b9744b498ac1bbac69950bc` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; the monorepo ref is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. Subsequent Dusk docs/report/guard commits are outside `DUSK_REPRO_COVERED_PATHS`, and the production-readiness guard reports `coveredPathDelta: none`. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF` and report `monorepoCoveredPathDelta`. Moving PR head and check URLs are intentionally read from the live PR check rollups rather than pinned in this audit row. |
