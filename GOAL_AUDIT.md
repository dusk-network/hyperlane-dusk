...
|---|---|---|---|
| `dusk-network/hyperlane-dusk` | `feat/dusk-hardening-v2` | Live PR head is checked through GitHub and `make gate-status`; latest clean-layout `make repro-check-agent` evidence is run `1778722626`, which tested Dusk source ref `222ae14469a960403699f5507f72be7c52321b29`, monorepo `515fab074024271935bc7795604dbb4f0823a937`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`. Previous clean-layout evidence run `1778695627` tested Dusk source ref `ef8ee43cd99569299b9744b498ac1bbac69950bc`, monorepo `515fab074024271935bc7795604dbb4f0823a937`, upstream base `7689ff65f4929a72ad0650a03e8dd7d987f0e802`, and local Rusk path dependencies; earlier current-head and clean-Rusk E2E evidence is retained in `TEST_REPORT.md`. Earlier commits cover implementation, cleanup regression guards, audit, CI/repro, signer, default-branch, cross-repo handoff docs, generated signer key cleanup, generated agent-config cleanup, gate-status reporting, split decision routing, reviewer evidence links, Mailbox fee-overflow hardening, Mailbox nonce-overflow hardening, fee-accounting overflow hardening, Dusk-side RUES client error propagation, targeted wasm clippy evidence for the production contract/type surface, repeatable `make clippy-contracts` wrapper coverage, checkout-v6 workflow hardening, repeatable GitHub review-hygiene guardrails including live PR-head validation, stale clean-layout evidence detection, Historical/Superseded review-comment filtering, secret-artifact guard extensions for single-quoted YAML/TOML key values plus uppercase env/log key assignments, and warp-route direct `unwrap()` removal plus a contract unwrap guard in `make gate-status`. `TEST_REPORT.md` records pushed secret-artifact guard evidence, dependency-alert guard evidence, warp unwrap hardening evidence, and representative post-push production-readiness blocker samples; moving PR head and check rollups are intentionally read live through GitHub and the gate scripts instead of pinned in this row. | Internal PR ready for review, open, blocked on review/sign-off; review requested from `moCello`; labels `need:feedback`, `type:feature`; `Dusk review policy gate` passing and required by default-branch protection; `Production readiness guard` failing as expected while production blockers remain |
| `dusk-network/hyperlane-monorepo` | `feat/dusk-support-v2` | Live PR head is checked through GitHub and `make gate-status`; upstream freshness is checked with `git fetch upstream main`, `git merge-base HEAD upstream/main`, and `git rev-list --left-right --count HEAD...upstream/main`; Dusk signer test cleanup evidence `b989bbcfbb2a427d3a538c5201f5d7214de6ba84`; upstream compatibility and upstream PR gate plan documented in `docs/dusk-upstream-compatibility-review.md` and `docs/dusk-upstream-pr-plan.md`; companion `TEST_REPORT.md` records latest clean-layout `make repro-check-agent` run `1778695627`, post-rebase clean-Rusk E2E runs `1778609411` and `1778609697`, and dependency-remediated clean-Rusk E2E runs `1778613709` and `1778613956`. Latest clean-layout repro passed for monorepo ref `515fab074024271935bc7795604dbb4f0823a937` after rebasing onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`; live monorepo PR head and `monorepoCoveredPathDelta` are checked through live PR status and `make gate-status-fresh` rather than pinned in this row. | Internal PR ready for review, open, blocked on review/provisioning; review requested from `Neotamandua`; labels `need:feedback`, `type:feature`; `Dusk review policy gate` passing and required by default-branch protection; `Dusk agent cargo check` failing at the private companion-repo preflight until `DUSK_ORG_READ_TOKEN` is provisioned; inherited Depot-backed checks now skip instead of staying queued in the Dusk fork; merge-base equals upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802` |
| `dusk-network/hyperlane-dusk` workflow dispatcher | `ci/manual-repro-workflow` | Live PR head is checked through GitHub and `make gate-status`; contains `.github/workflows/manual-repro-check.yml`, `.github/workflows/manual-repro-dispatcher-gate.yml`, `.github/workflows/dusk-review-policy-gate.yml`, and `.github/actionlint.yaml`; the manual workflow logs requested refs and resolved checkout heads, then runs the final repro step under explicit `shell: bash` with `set -euo pipefail` before `make repro-check-agent` | Workflow-only default-branch PR #3 ready for review, open, blocked on review; review requested from `moCello` and `Neotamandua`; labels `need:feedback`, `type:docs`; `Manual repro dispatcher gate` and `Dusk review policy gate` passing |
...
`DUSK_ORG_READ_TOKEN` is provisioned. A later docs-only monorepo head
`9050143c1ef12f76d117ee97effa79da8df3e334` and later
`fc9a2eaec28a8226497c7d678fd591f9d274b6f8` refresh upstream-prep docs
without changing runtime code. The current monorepo head
`515fab074024271935bc7795604dbb4f0823a937` rebases onto upstream
`7689ff65f4929a72ad0650a03e8dd7d987f0e802`, an npm release metadata commit
that does not touch the Dusk Rust agent/runtime surface. Later
`515fab074024271935bc7795604dbb4f0823a937` also updates the Dusk agent gate
and Dusk review-policy gate to `actions/checkout@v6` after the tag was verified through the GitHub API.
The live monorepo PR rollup now has no queued checks; the one expected Dusk
agent failure remains the private companion-repo preflight.

Dusk-side runtime and secret-artifact hardening on 2026-05-13 moved
`dusk-network/hyperlane-dusk` to
`8d3704e8f5a3ab0976b97fc3a68319e112e8affc`. The earlier
`93ed3b07b26d8784610d2ba754a851490de15e21` line makes the `dusk-tx` and
`e2e` RUES clients propagate `reqwest::Client` build errors instead of
panicking during client construction. The later `8d3704e` line cleans
generated Hyperlane agent config files from E2E wrappers and extends
`make secret-hygiene` to require that cleanup path for future
`gen-agent-configs.sh` consumers. Focused syntax/hygiene checks and
`git diff --check` passed before clean-layout repro run `1778683232` covered
...
- Manual workflow dispatcher PR #3 is ready for review, open, has
  ...
  contains only workflow/actionlint files needed to make manual
  `workflow_dispatch` available from the default branch after Dusk review while
...
- `make production-readiness-guard` is a negative guard that must fail while
  ...
  branch protection, insufficient required status-check count, and wrong
  required status-check contexts with `BRANCH_PROTECTION_GATE_ONLY=1`, and
  missing workflow visibility, missing runner, and `DUSK_ORG_READ_TOKEN`
  provisioning with a mocked GitHub API response through
  `CI_VISIBILITY_GATE_ONLY=1`. It is not a
...
- Default-branch dispatcher PR dusk-network/hyperlane-dusk#3 now exists with
  only workflow/actionlint files: manual repro workflow, dispatcher self-check,
  shared review-policy gate, and actionlint config. It can make the workflow
...
- `make gate-status` prints the current machine-checkable gate status:
  ...
  policy enabled for Dusk `Dusk review policy gate` + `Production readiness
  guard` and monorepo `Dusk review policy gate` + `Dusk agent cargo check`
  with `missingRequiredStatusChecks: none`, repo-level Actions secret
...
| Address arithmetic, replay/domain separation, malformed metadata, duplicate delivery, admin paths, dirty redeploys, and token accounting | `SECURITY_REVIEW.md`; `TEST_REPORT.md`; `Cargo.toml` release `overflow-checks = true`; 72 integration tests; E2E/fault-injection runs listed below | Done for internal review |
| Avoid runtime `todo!`, `unimplemented!`, and direct `panic!` paths | `TEST_REPORT.md` records the Dusk contract/runtime/tooling scan command and result for `rg -n "todo!|unimplemented!|panic!" contracts types data-driver dusk-tx e2e wasm-bindings demo -g '!target'` with no matches in scoped production paths. `make gate-status` now also scans the tracked Dusk repo paths `contracts types data-driver dusk-tx e2e wasm-bindings demo`, rejects direct `.unwrap()` in `contracts/`, and scans the monorepo `rust/main/chains/hyperlane-dusk/src` crate for direct placeholder/panic macros and `expect\(` in Dusk agent runtime source. Dusk commit `eff5e3bc181707756eead42880c71ccd1e685d34` removes direct `unwrap()` from the three production warp inbound-router checks, and `make fail-closed-self-test` covers the contract-unwrap scan failing closed on invalid patterns. The monorepo head `48dcc0c87efc12584904d841d5088c1ae4acef20` contains the Dusk agent `expect(...)` removals from `rust/main/chains/hyperlane-dusk/src`; local `git grep -n -E 'todo!|unimplemented!|panic!|expect\(' -- rust/main/chains/hyperlane-dusk/src`, `make review-hygiene`, and the updated CI scan report no matches before the CI job reaches the expected private Dusk checkout token preflight. The monorepo `docs/dusk-upstream-compatibility-review.md` records no added placeholder macros in the Dusk diff against `upstream/main`. | Done for internal review |
| Avoid leaking Dusk secrets through committed configs and process argv | Demo/E2E configs use ignored local files and `/tmp` artifacts; `duskKey` now supports `keyFile`/`keyEnv` so Dusk raw keys do not need to be embedded in generated JSON; `keyFile` rejects non-regular files and, on Unix, group/world-readable files before reading key material; demo agent configs reference `0600`-style `/tmp/*.key` files through `keyFile`; E2E wrappers remove generated Dusk signer key files and generated agent config files on exit after stopping agents, and `make secret-hygiene` now guards future `gen-agent-configs.sh` consumers for both cleanup paths; `dusk-tx` supports password file/env lookup and stdin raw-key loading; demo scripts avoid Dusk consensus passwords in CLI argv; exported GitHub-facing PR/issue text passed `scripts/secret-hygiene-check.sh`; `SECRET_HANDLING.md` and `make secret-hygiene` provide source/artifact guardrails; `SECURITY_REVIEW.md` keeps production signer/config handling as a release gate | Gated |
| Dusk VM contract/type checks | `make all`; `make clippy-contracts` for the production contract/type surface -> passed; `cargo test -p hyperlane-dusk-types` -> 28 passed; `cargo test -p hyperlane-dusk-integration-tests` -> 72 passed; `cargo test -p dusk-tx` -> 3 passed; latest clean-layout `make repro-check-agent` result `1778722626` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
| Dusk Rust dependency advisories | GitHub Dependabot alerts were triaged against the active dependency graph. `Cargo.toml`/`Cargo.lock` now move the reported vulnerable Rust packages to patched versions where applicable: `wasmtime` 36.0.9, `openssl` 0.10.79, `rustls-webpki` 0.103.13, `quinn-proto` 0.11.14, `rand` 0.8.6/0.9.3, and `keccak` 0.1.6; `lru` was already 0.16.4. Dependency remediation run `1778613424` passed 28 type tests, 3 `dusk-tx` tests, 70 VM integration tests, CLI/E2E compile checks, dependency-tree probes, and secret hygiene. Clean-Rusk E2E runs `1778613709` and `1778613956` passed the TestMock and MessageIdMultisig bidirectional bridge paths on the updated graph. `make dependency-alert-status` compares open GitHub Dependabot `Cargo.lock` alerts against the current local lockfile and confirms no local locked package versions fall within the alert vulnerable ranges; `make production-readiness-guard` blocks if dependency-alert triage is unavailable or reports vulnerable, unparsed, or unpatchable open alerts; `make fail-closed-self-test` mocks unavailable dependency-alert triage through `DEPENDENCY_ALERT_GATE_ONLY=1`; `make gate-status` also reports current Dusk Dependabot open-alert visibility so reviewers can distinguish the patched local graph from GitHub's rescan/default-branch state. | Done for internal review |
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `7689ff65`; latest clean-layout `make repro-check-agent` result `1778722626` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
| Local EVM <-> Dusk with TestMock/null-style ISM | Clean Rusk run `1778520709`: bidirectional E2E passed; review-head clean-Rusk run `1778587094` passed; post-rebase clean-Rusk run `1778609411` passed; dependency-remediated clean-Rusk run `1778613709` passed with monorepo `1f9e49fd9f0ba84ea93e472ddbd31fddd9a04cc3`; current-runtime clean-Rusk run `1778672800` passed with Dusk PR head `fd2ec8fe996dd28f85259e6aebd8db25182c20de` and monorepo `48dcc0c87efc12584904d841d5088c1ae4acef20` | Done |
| Local EVM <-> Dusk with MessageIdMultisigISM | Clean Rusk run `1778521018`: bidirectional E2E with validator/checkpoint passed; review-head clean-Rusk run `1778587351` passed; post-rebase clean-Rusk run `1778609697` passed; dependency-remediated clean-Rusk run `1778613956` passed with monorepo `1f9e49fd9f0ba84ea93e472ddbd31fddd9a04cc3`; current-runtime clean-Rusk run `1778673269` passed with Dusk PR head `fd2ec8fe996dd28f85259e6aebd8db25182c20de` and monorepo `48dcc0c87efc12584904d841d5088c1ae4acef20` | Done |
| Bidirectional DUSK/wDUSK bridge through relayer/validator, no manual process calls | `TEST_REPORT.md` records relayer/validator-driven EVM -> Dusk and Dusk -> EVM flows for TestMock and MessageIdMultisig; current-runtime clean-Rusk runs `1778672800` and `1778673269` passed both directions after the RUES client hardening and docs/guard-only head updates | Done |
| E2E freshness against current runtime source | `TEST_REPORT.md` records current-runtime clean-Rusk TestMock and MessageIdMultisig E2E runs `1778672800` and `1778673269` at Dusk PR head `fd2ec8fe996dd28f85259e6aebd8db25182c20de`, latest covered Dusk implementation ref `93ed3b07b26d8784610d2ba754a851490de15e21`, monorepo `48dcc0c87efc12584904d841d5088c1ae4acef20`, upstream base `2b7db706023806b36a57e446205ae443537ae9ec`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc` | Done |
...
| Prepare upstream Hyperlane draft PRs only after internal review | Not started by design; PR bodies explicitly state upstream prep waits for internal Dusk review. `make gate-status-fresh` reports open `hyperlane-xyz/hyperlane-monorepo` PRs from `dusk-network:feat/dusk-support-v2`; `make production-readiness-guard` fails as premature if any are visible while internal blockers remain; `make fail-closed-self-test` covers the premature-upstream-PR blocker with a mocked search response. | Gated |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484 tied to their test coverage; `make report-hygiene` rejects stale decision-record links to the superseded clean-layout evidence comment |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; artifact scans reject secret-like key/pem filenames plus `.env`/`.env.*` files before upload and reject inline `key`, `privateKey`, or `private_key` 32-byte hex fields in JSON, TOML, and YAML-style assignments plus uppercase env/log key assignments; `make fail-closed-self-test` covers `.env.bridge`, JSON `privateKey`, TOML `private_key`, single-quoted YAML `privateKey`, and `DUSK_SIGNER_KEY` artifact rejection; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
| Extracted evidence archive hygiene | `make archive-hygiene-test`; `make archive-hygiene`; on 2026-05-13 all durable `.tgz` evidence archives in `/home/hein_/projects/hyperlane/.codex-backups` were extracted into a temporary directory and scanned with `bash scripts/secret-hygiene-check.sh "$scan_root"`; the extracted-content scan covered repro, E2E, dependency-remediation, and soak handoff archives and found no secret-like filenames or signer/password command text. The wrapper verifies tracked checksums from `EVIDENCE_ARCHIVES.sha256` when present, rejects unsafe manifest paths, unsafe fallback `ARCHIVE_EXPECTED_SHA256S` paths, non-`.tgz` fallback expected-SHA names, archive-directory/manifest drift, unsafe member paths, non-regular/non-directory archive members, and extracted symlinks or special files; the self-test covers safe archives plus archive SHA256 mismatch, unsafe expected-SHA entries, non-archive expected-SHA entries, unsafe manifest entries, unmanifested archives, traversal, symlink, and secret-bearing rejection paths; the exact command shape is recorded in `TEST_REPORT.md`. |
| Reviewer-facing and report link guardrails | `scripts/github-review-hygiene.sh`, `scripts/report-hygiene-check.sh`, `scripts/release-gate-status.sh`, `scripts/dependency-alert-status.sh`, `scripts/completion-audit-status.sh`, `scripts/production-readiness-guard.sh`, `make review-hygiene`, `make report-hygiene`, `make dependency-alert-status`, `make completion-audit-status`, `make gate-status`, `make gate-status-fresh`, `make review-gates`, `make production-readiness-guard`; Dusk PR #1, monorepo PR #1, and sign-off issue #2 bodies must include or visibly report latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484, run `1778722626`, Dusk source ref `222ae14469a960403699f5507f72be7c52321b29`, monorepo ref `515fab074024271935bc7795604dbb4f0823a937`, clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`, dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389, post-rebase E2E/archive links, the advisory reviewer routing URL https://github.com/dusk-network/hyperlane-dusk/blob/feat/dusk-hardening-v2/REVIEWERS.md, current head/gate refresh handoff text, `make gate-status-fresh`, `make dependency-alert-status`, `make completion-audit-status`, `make review-gates`, archive hygiene self-tests, extracted evidence archive hygiene scans, `make production-readiness-guard`, required status-check policy enabled, and latest clean-layout repro path delta; workflow PR #3 must include or visibly report the advisory reviewer routing URL and the #8 CI provisioning runbook https://github.com/dusk-network/hyperlane-dusk/issues/8#issuecomment-4435830841, and PR #3 comments must carry the current clean-layout repro evidence link instead of the stale dispatcher evidence link https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4440412895. Active bodies/comments must not include the superseded `1778607202`/`4433179148` clean-layout repro evidence, the stale `1778683232`/`9050143c1ef12f76d117ee97effa79da8df3e334` monorepo latest-repro pair in PR/issue bodies, stale "Latest checkout-v6 clean-layout" wording, stale gate-refresh head/check URLs such as `8b15eb607e83b80cc334c6402e6c752dcfc9a1ba`, moving Dusk or monorepo PR-head/check-run pins in active comments, the stale pre-archive-hygiene `make review-gates` bundle description, stale monorepo queued-check status from before the inherited Depot workflow guards, stale Dusk handoff head/check URLs from earlier gate refreshes, or the active-current wording `Current latest clean-layout repro evidence`. Issue #2 must keep the current `1778722626`/`222ae14469a960403699f5507f72be7c52321b29`/`515fab074024271935bc7795604dbb4f0823a937` latest clean-layout repro handoff text. Local reports must not reintroduce the stale Dusk CI URLs, dispatcher merge-order implementation head, monorepo success count, old export paths, stale latest-repro link that were corrected during the current report refresh, or stale production-decision evidence links to superseded clean-layout repro comments. Moving Dusk and monorepo PR check URLs are intentionally read from live PR check rollups instead of pinned in comments, and `make review-hygiene` intentionally no longer requires a fixed "current gate refresh" comment URL. `make report-hygiene` also verifies the latest durable repro archive file exists locally and matches its expected SHA256 before accepting report mentions as evidence. |
| CI/repro runner proposal | `CI_REPRO_STRATEGY.md`, including the `Admin Provisioning Runbook`; `SECRET_HANDLING.md`; `PRODUCTION_REVIEW_DECISIONS.md`; `.github/workflows/manual-repro-check.yml`; `.github/workflows/production-readiness-gate.yml`; `make repro-check-agent`; `make gate-status`; `make gate-status-fresh`; `make production-readiness-guard`; `make fail-closed-self-test`; dusk-network/hyperlane-dusk#3; dusk-network/hyperlane-dusk#8; latest #8 runbook comment https://github.com/dusk-network/hyperlane-dusk/issues/8#issuecomment-4435830841; latest #8 CI provisioning guardrail comment https://github.com/dusk-network/hyperlane-dusk/issues/8#issuecomment-4445712911. Issue #8 now ties the latest clean-layout repro evidence link to Dusk source ref `222ae14469a960403699f5507f72be7c52321b29`, monorepo ref `515fab074024271935bc7795604dbb4f0823a937`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`, and `make review-hygiene` rejects the stale older tested-ref text there. Required status-check promotion is complete as of 2026-05-14: Dusk requires `Dusk review policy gate` + `Production readiness guard`, monorepo requires `Dusk review policy gate` + `Dusk agent cargo check`, and `make gate-status` reports `missingRequiredStatusChecks: none` for both repos. The docs scope `DUSK_ORG_READ_TOKEN` to read-only source checkout for `dusk-network/hyperlane-dusk`, `dusk-network/hyperlane-monorepo`, and `dusk-network/rusk-private`, and explicitly exclude signer/runtime/image-publishing uses. Branch protection, Actions-secret, runner-admin, and Dependabot-alert visibility are separated as production-readiness status gates that need approved admin/security-read visibility rather than broader source-checkout token scope. The production-readiness workflow can use optional `DUSK_STATUS_READ_TOKEN` for status visibility only; the manual repro checkout workflow continues to use `DUSK_ORG_READ_TOKEN`. `make fail-closed-self-test` mocks the GitHub branch-protection/workflow/runner/secret APIs and verifies the guard rejects missing default branch protection, missing workflow visibility, missing `dusk-hyperlane` runner, missing `DUSK_ORG_READ_TOKEN` provisioning, unavailable dependency-alert triage, premature upstream PRs, and latest-repro covered-path deltas such as the malformed warp-token VM test slice in `tests/tests/integration.rs`. |
...
| Previous clean-layout repro evidence archive | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-agent-config-cleanup-repro-1778683232.tgz`; SHA256 `d7459059cf413d6d6a7c404248429e760aca0a7fd4ab784a5cb068fb63c7df0b`; contains clean-layout repro run `1778683232` for Dusk source ref `8d3704e8f5a3ab0976b97fc3a68319e112e8affc` and monorepo ref `9050143c1ef12f76d117ee97effa79da8df3e334`; `scripts/secret-hygiene-check.sh` passed over the repro log, and `scripts/archive-hygiene-check.sh` passed over the evidence archive directory after the archive was added |
| Latest clean-layout repro evidence | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-clean-repro-covered-tests-1778722626.tgz`; SHA256 `1c4129a1113521817549101f9180ac9d3854035fcac93cbef570f4e8ef618649`; contains `/tmp/hyperlane-clean-repro-covered-tests-1778722626.log` with SHA256 `bf43bb455f8634c17496219e6d319cd7baca312e93b384c6225f32142b6c43e3`; contains clean-layout repro run `1778722626` for Dusk source ref `222ae14469a960403699f5507f72be7c52321b29` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; `scripts/secret-hygiene-check.sh` passed inside the repro and over the archive staging directory and tarball, and `scripts/archive-hygiene-check.sh` passed over the durable evidence archive directory after the archive was added. GitHub-facing evidence comment: https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484. Previous checkout-v6 evidence remains in issue #2 history. |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778722626` covers Dusk source ref `222ae14469a960403699f5507f72be7c52321b29` and monorepo ref `515fab074024271935bc7795604dbb4f0823a937`; the monorepo ref is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. `DUSK_REPRO_COVERED_PATHS` includes runtime, tooling, demo, and VM integration test source (`contracts types data-driver dusk-tx e2e wasm-bindings demo tests Cargo.toml Cargo.lock`) so test-source changes cannot be hidden from repro-delta reporting. The malformed warp-token VM test slice in `tests/tests/integration.rs` is covered by run `1778722626`; subsequent Dusk docs/report/guard commits are outside that covered path set, so `coveredPathDelta` should remain `none` until another covered source/test path changes. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF` and report `monorepoCoveredPathDelta`. Moving PR head and check URLs are intentionally read from the live PR check rollups rather than pinned in this audit row. |
...
| Recent head and gate refresh | Active Dusk PR #1, monorepo PR #1, sign-off issue #2, and split decision issue bodies point at latest clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484 and the current gate handoff https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446717505. A 2026-05-14 live refresh after promoting repro run `1778722626` tested Dusk source ref `222ae14469a960403699f5507f72be7c52321b29`; later docs/report/guard-only commits moved the live Dusk PR head beyond that tested source ref, so current head is read live from GitHub instead of pinned here. Latest aggregate `make review-gates` passed with review-hygiene export `/tmp/hyperlane-review-export-1778724892` and dispatcher merge-order log `/tmp/hyperlane-merge-order-logs.uXBq9C`; follow-up GitHub handoff refreshes passed review-hygiene exports `/tmp/hyperlane-review-export-1778724988`, `/tmp/hyperlane-review-export-1778725055`, and `/tmp/hyperlane-review-export-1778725111`; after recording those in `TEST_REPORT.md`, Dusk PR #1 checks settled with `Dusk review policy gate` passing and `Production readiness guard` failing closed, and `make review-hygiene` passed with export `/tmp/hyperlane-review-export-1778725258`. A fresh upstream check confirmed Hyperlane upstream base `7689ff65f4929a72ad0650a03e8dd7d987f0e802` is still the merge-base and the branch remains `44 0` ahead/behind. `make production-readiness-guard` failed closed on the expected blockers: open/review-required Dusk, monorepo, and dispatcher PRs; 7 unchecked issue #2 sign-off items; split decision issues #4-#9 open; no visible repo-level `dusk-hyperlane` runner; and no visible `DUSK_ORG_READ_TOKEN` in either internal repo. Moving PR heads and check URLs are intentionally read live with `make gate-status`, `make gate-status-fresh`, and `gh pr view 1 --repo dusk-network/hyperlane-dusk --json headRefOid,statusCheckRollup,reviewDecision,mergeable,state` instead of being treated as static completion proof. |
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4446653484 |
...
3. Production secret handling needs an operational/CI sign-off. Current scripts
   keep dev configs and Dusk signer key files in ignored `/tmp` paths only while
   agents run, generated Dusk agent configs use `duskKey.keyFile` instead of
   inline Dusk raw keys, the agent rejects non-regular and group/world-readable
   key files on Unix, Dusk consensus passwords are no longer passed to
   `dusk-tx` in process argv, and `make secret-hygiene` provides
   source/artifact guardrails.
...
6. CI/repro runner strategy remains a release gate. The internal PRs now have
   ...
   enabled for Dusk `Dusk review policy gate` + `Production readiness guard`
   and monorepo `Dusk review policy gate` + `Dusk agent cargo check`; the
   latest `make gate-status` sample reported `missingRequiredStatusChecks: none`,
   so remaining CI/repro blockers are acceptance, PR #3/default-branch
   workflow publication, token/runner/status visibility, and passing exact-ref
   runs. The narrow default-branch dispatcher PR is
