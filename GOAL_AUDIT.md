...
| Live-head delta after latest clean-layout repro | The live Dusk PR head is checked through GitHub and `make gate-status`. From tested Dusk source ref `836ee7d8d8e95152b3daaeebbc3fb56b0cc8e253` to the dependency-remediated branch update, `git diff --name-status` now includes `Cargo.toml`, `Cargo.lock`, `GOAL_AUDIT.md`, `TEST_REPORT.md`, and reviewer/gate hygiene scripts. Focused dependency remediation run `1778613424` and clean-Rusk E2E runs `1778613709` and `1778613956` cover the updated Rust dependency graph. |
