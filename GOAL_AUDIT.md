...
`cdb98dcb0c78daa400cb424115779b508c89e9da` rebases onto upstream
`215135227a0e47883d3581433a02c68d89986e41`, whose new ICA helper subpath
export is TypeScript SDK-only for the Dusk integration. The live monorepo PR
rollup now has no queued checks: 14 completed successes, 22 completed skips,
and the one expected Dusk agent failure.
...
- Dusk PR is ready for review and open; review is requested from `moCello`;
  live head `d7a0a35ebc01a8790ca252fac215608b76559190` has
  `Dusk review policy gate` passing at
  https://github.com/dusk-network/hyperlane-dusk/actions/runs/25812841609/job/75833452017
  ...
  The failed-job summary reaches the known external blockers and reports
...
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `2b7db706`; latest clean-layout `make repro-check-agent` result `1778683232` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Reviewer decision record | `PRODUCTION_REVIEW_DECISIONS.md`; split contract-policy issues #4 through #6 must keep current clean-layout repro evidence https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4442208973 tied to their test coverage |
...
| Secret handling policy, signer custody proposal, and source/artifact guardrail | `SECRET_HANDLING.md`, `PRODUCTION_SIGNER_POLICY.md`, `scripts/secret-hygiene-check.sh`, `scripts/github-review-hygiene.sh`, `make secret-hygiene`, `make review-hygiene`; dusk-network/hyperlane-dusk#7 must keep current signer-custody evidence links to latest clean-layout repro https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4442208973 and dependency-remediated E2E https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4434118389 |
...
| Latest clean-layout repro evidence archive | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-agent-config-cleanup-repro-1778683232.tgz`; SHA256 `d7459059cf413d6d6a7c404248429e760aca0a7fd4ab784a5cb068fb63c7df0b`; contains clean-layout repro run `1778683232` for Dusk source ref `8d3704e8f5a3ab0976b97fc3a68319e112e8affc` and monorepo ref `9050143c1ef12f76d117ee97effa79da8df3e334`; `scripts/secret-hygiene-check.sh` passed over the repro log, and `scripts/archive-hygiene-check.sh` passed over the evidence archive directory after the archive was added |
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778683232` covers Dusk implementation source ref `8d3704e8f5a3ab0976b97fc3a68319e112e8affc` and validated monorepo runtime ref `9050143c1ef12f76d117ee97effa79da8df3e334`; the current monorepo PR head `cdb98dcb0c78daa400cb424115779b508c89e9da` is rebased onto upstream `215135227a0e47883d3581433a02c68d89986e41`. Subsequent Dusk docs, guard, and decision-record commits are outside `DUSK_REPRO_COVERED_PATHS`, and the production-readiness guard reports `coveredPathDelta: none`. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF`; current output reports `monorepoCoveredPathDelta: none` and `monorepoAllPathDelta` limited to upstream TypeScript SDK/infra files plus the two upstream-prep docs. |
...
| Latest clean-layout repro evidence | https://github.com/dusk-network/hyperlane-dusk/issues/2#issuecomment-4442208973 |
