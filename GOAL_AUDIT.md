...
`f6b131223052300c34c3f58c87042486368ea6ab` rebases onto upstream
...
| Live-head delta after latest clean-layout repro | The live Dusk and monorepo PR heads are checked through GitHub and `make gate-status`. Latest clean-layout repro run `1778683232` covers Dusk implementation source ref `8d3704e8f5a3ab0976b97fc3a68319e112e8affc` and validated monorepo runtime ref `9050143c1ef12f76d117ee97effa79da8df3e334`; the current monorepo PR head `f6b131223052300c34c3f58c87042486368ea6ab` is rebased onto upstream `7689ff65f4929a72ad0650a03e8dd7d987f0e802`. Subsequent Dusk docs, guard, and decision-record commits are outside `DUSK_REPRO_COVERED_PATHS`, and the production-readiness guard reports `coveredPathDelta: none`. `make gate-status-fresh` and `make production-readiness-guard` also compare the live monorepo PR head against `LATEST_REPRO_MONOREPO_REF`; current output reports `monorepoCoveredPathDelta: none` while `monorepoAllPathDelta` is limited to upstream TypeScript SDK/infra changes, upstream TypeScript/Starknet release metadata, Dusk upstream-prep docs, and the Dusk-fork rebalancer-sim workflow guard. |
