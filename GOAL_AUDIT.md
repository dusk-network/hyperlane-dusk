...
| Avoid leaking Dusk secrets through committed configs and process argv | Demo/E2E configs use ignored local files and `/tmp` artifacts; `duskKey` now supports `keyFile`/`keyEnv` so Dusk raw keys do not need to be embedded in generated JSON; `keyFile` rejects non-regular files and, on Unix, group/world-readable files before reading key material; demo agent configs reference `0600`-style `/tmp/*.key` files through `keyFile`; `dusk-tx` supports password file/env lookup and stdin raw-key loading; demo scripts avoid Dusk consensus passwords in CLI argv; `SECRET_HANDLING.md` and `make secret-hygiene` provide source/artifact guardrails; `SECURITY_REVIEW.md` keeps production signer/config handling as a release gate | Gated |
