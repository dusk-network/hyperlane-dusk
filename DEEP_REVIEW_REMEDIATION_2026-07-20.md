...
| C41: policy self-tests must be hermetic | Give the clean-repro delta and completion-audit source checks narrow gate-only entry points, and exercise agent scanning against a temporary tracked Git fixture. Negative tests now reach their intended assertions without GitHub authentication, a sibling monorepo checkout, archive refs, or backup artifacts; each full production path invokes the same factored check. Git worktree metadata is accepted whether `.git` is a directory or a linked-worktree file. |
