...
|---|---|---|---|
| `dusk-network/hyperlane-dusk` | `feat/dusk-hardening-v2` | Live PR head is checked through GitHub and `make gate-status`; latest clean-layout `make repro-check-agent` evidence is run `1778574482`, which tested Dusk `28d07e01d1bbc0cf59575a811cde55e844a2abb7`, monorepo `a44020dc998b7fe868254a5d1a349b9eb8ded899`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`; earlier commits cover implementation, cleanup regression guards, audit, CI/repro, signer, default-branch, cross-repo handoff docs, generated signer key cleanup, gate-status reporting, split decision routing, reviewer evidence links, Mailbox fee-overflow hardening, Mailbox nonce-overflow hardening, and fee-accounting overflow hardening | Internal PR ready for review, open, mergeable; review requested from `moCello`; labels `need:feedback`, `type:feature`; no status checks configured |
...
| Address arithmetic, replay/domain separation, malformed metadata, duplicate delivery, admin paths, dirty redeploys, and token accounting | `SECURITY_REVIEW.md`; `TEST_REPORT.md`; 70 integration tests; E2E/fault-injection runs listed below | Done for internal review |
...
| Dusk VM contract/type checks | `make all`; `cargo test -p hyperlane-dusk-types` -> 28 passed; `cargo test -p hyperlane-dusk-integration-tests` -> 68 passed; `cargo test -p dusk-tx` -> 3 passed; latest clean-layout `make repro-check-agent` result with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
