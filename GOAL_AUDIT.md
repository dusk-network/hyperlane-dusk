...
|---|---|---|---|
| `dusk-network/hyperlane-dusk` | `feat/dusk-hardening-v2` | Live PR head is checked through GitHub and `make gate-status`; latest clean-layout `make repro-check-agent` evidence is run `1778561743`, which tested Dusk `c2d5d946cf08ce7c48ff373350989512f07dacc6`, monorepo `dea286bd364a9268413fd5b1cfc51bd983d443be`, and clean Rusk `c0c64db4659500d077bb253ad13acba0e347d3fc`; earlier commits cover implementation, cleanup regression guards, audit, CI/repro, signer, default-branch, cross-repo handoff docs, generated signer key cleanup, gate-status reporting, split decision routing, and reviewer evidence links | Internal PR ready for review, open, mergeable; review requested from `moCello`; labels `need:feedback`, `type:feature`; no status checks configured |
