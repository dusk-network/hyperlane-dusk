...
| Dusk VM contract/type checks | `make all`; `make clippy-contracts` for the production contract/type surface -> passed; `cargo test -p hyperlane-dusk-types` -> 28 passed; `cargo test -p hyperlane-dusk-integration-tests` -> 70 passed; `cargo test -p dusk-tx` -> 3 passed; latest clean-layout `make repro-check-agent` result `1778599935` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
| Hyperlane Rust agent checks | From monorepo `rust/main`: `cargo check -p hyperlane-dusk -p hyperlane-base -p validator -p relayer -p scraper -p lander` -> passed after rebasing onto upstream `7a362a09`; latest clean-layout `make repro-check-agent` result `1778599935` with exact tested SHAs is recorded in `TEST_REPORT.md` | Done |
...
| Latest clean-layout repro evidence archive | `/home/hein_/projects/hyperlane/.codex-backups/hyperlane-current-head-repro-1778599935.tgz`; SHA256 `4ac0ba327024358bed5278fd276f796d71cbb6940c8417fdc76125cfedfdfa44`; contains clean-layout repro run `1778599935` for Dusk source ref `de9b7fa3c832fb60982d3dbd22c8a59114d37732`; `scripts/secret-hygiene-check.sh` passed over the repro log, archive staging directory, and tarball |
